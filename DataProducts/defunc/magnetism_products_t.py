...
part0 = False # check for availability of paths
part1 = False # analysis of primary sensors -> adjusted data
...
if part3 and part1:
    ...
    if runqd:
        try:
            ...
            statusmsg[name3b] = "QD data between {} and {} calculated and published".format(starttime, endtime)
...
if part4 and part3 and runqd:
    ...
    tmax = datetime(2019,11,30)
    tmin = datetime(2019,11,1)
    qddays = (tmax-tmin).days
    qdlist = [datetime.strftime(tmax-timedelta(days=i),"%Y%m%d") for i in range(0,qddays,1)]
    ...
    try:
        ...
        for da in qdlist:
            ...
            if submit2gin:
                if not active_pid('curl'):
                    print ("Data from {}".format(da))
                    print ("----------------")
                    if second:
                        print ("  -- Uploading second data to GIN - active now")
                        uploadsec = ginupload(os.path.join(qpathsec,'wic'+da+'qsec.sec'), ginuser, ginpasswd, ginaddress,stdout=True)
                        print ("  -> Answer: {}".format(uploadsec))
                    if minute:
                        print ("  -- Uploading minute data to GIN: {}".format(da))
                        uploadmin = ginupload(os.path.join(qpathmin,'wic'+da+'qmin.min'), ginuser, ginpasswd, ginaddress,stdout=True)
                        print ("  -> Answer: {}".format(uploadmin))

                    statusmsg[name4] = 'upload of QD successful'
    except:
